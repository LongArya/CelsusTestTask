...
Так как в дебажном callback были зафиксированы нулевые градиенты, исследовал эту проблему подробнее, чтобы было проще следить за качеством визуально, немного модицифировал архитектуру сиамской сети, добавив туда хед который маппит ебмеддинг в 2d. Добавил дебажный callback, который отрисовывает эмбеддинги валидационного датасета после каждой итерации и попробовал оверфитнуться на датасете из одной отрицательной пары.  

[Пример_рана](https://app.clear.ml/projects/fcd61b2d4f8f4a19b16d82e73646d804/experiments/8f1743991a104b388c8af755be64c616/output/execution)   
Отрисовки можно посмотреть во вкладке Plots/Debug_embedding_plots     

В итоге увидел 2 момента:  
1. Эмбеддинги у этой пары почти совпали, а на коллинеарных эмбеддингах плохо работает CosineEmbeddingLoss поэтому понятно почему их не получалось разделить. Серьезный минус CosineEmbeddingLoss, но возможно для этой задачи ее получится использовать если немного модифицировать. Возможные модификации, нацеленные на слом коллинеарности:  
1.1 Добавлять случайный небольшой шум к эмбеддингам перед подсчетом лосса  
1.2 Скейлить loss для разных пар с высоким cosine_similarity    
1.3 При высокой cosine_similarity задействовать в loss евклидово расстояние, чтобы на его основе немного разделить эмбеддинги, а потом уже окончательно разделить через CosineEmbeddingLoss   

2. Странно, что эмбеддинги почти совпали, потому что изображения разные, а веса на первых этапах еще случайные. Продебажил backbone, нашел ошибку. Перед global_average_pooling веса были похожи на распределение с мат ожиданием в 0, в итоге после average poolong эмбеддинг почти полностью состоял из 0. Добавил relu перед avg pooling.  

После запуска с этими улучшениями еще раз посмотрел на нормы градиентов, они перестали быть нулевыми, удалось хотя бы немного разделить эмбеддинги тестового датасета из одной негативной пары.  


Все наработки, залил в отдельную ветку 21_02_25_debug  
